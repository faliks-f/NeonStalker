...
constexpr double SAMPLING_FREQ = 20.0f; // 采样频率
...
float invSqrt(float number) {
    ...
    i = *reinterpret_cast<volatile uint32_t *>(&y);
    i = 0x5f375a86 - (i >> 1);

    y = *reinterpret_cast<volatile float *> (&i);
    y = y * (f - (x * y * y));
...
void quaternionSolution(float gx, float gy, float gz, float ax, float ay, float az) {
    float recipNorm;
    ...
    if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
        ...
        // 计算并应用积分反馈（如果启用）
        if (twoKi > 0.0f) {
            integralFBx += static_cast<float>(twoKi * halfex * (1.0f / SAMPLING_FREQ));  // integral error scaled by Ki
            integralFBy += static_cast<float>(twoKi * halfey * (1.0f / SAMPLING_FREQ));
            integralFBz += static_cast<float>(twoKi * halfez * (1.0f / SAMPLING_FREQ));
