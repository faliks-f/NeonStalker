...
class C_PiperRosNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.create_subscription(PosCmd, 'pos_line_cmd', self.pos_line_callback, 10)
        ...
        self.create_subscription(Float64, 'gripper_ctrl', self.gripper_ctrl_callback, 10)

        self.publisher_thread = threading.Thread(target=self.publish_thread)
        self.publisher_thread.start()

        self.get_logger().info(f"init success!")
    ...
    def pos_callback(self, pos_data):
        ...
        if(self.GetEnableFlag()):
            ...
            # self.piper.MotionCtrl_2(0x01, 0x00, 30)
            #

    def pos_line_callback(self, pos_data):
        """机械臂末端位姿订阅回调函数
    ...
    def gripper_ctrl_callback(self, dis):
        factor = 1000
        if dis.data >= 70:
            gripper_effort = 1
        else:
            gripper_effort = 3
        self.piper.GripperCtrl(round(abs(dis.data)*factor), gripper_effort * factor, 0x01, 0)
    ...
    def enable_callback(self, enable_flag:Bool):
        ...
        if(enable_flag.data):
            ...
            if(self.girpper_exist):
                self.piper.GripperCtrl(100000,1000,0x00, 0)
